...
export function LotMapDrawer({
  ...
  return (
    <>
      ...
      <aside
        className="fixed lg:absolute bottom-0 right-0 z-40 w-full lg:w-[370px] max-h-[85vh] lg:max-h-full h-auto lg:h-full bg-white border-t lg:border-t-0 lg:border-l border-slate-200 shadow-2xl flex flex-col rounded-t-xl lg:rounded-none overflow-hidden transition-all"
        aria-label="Ficha do Lote"
      >
        {/* Mobile handle indicator */}
        <div className="w-10 h-1 bg-slate-300 rounded-full mx-auto mt-2 mb-0.5 lg:hidden" />

        {/* Drawer Header */}
        <div className="p-3 sm:p-3.5 border-b border-slate-100 flex items-start justify-between bg-slate-50/70">
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] font-bold uppercase tracking-wider text-[#0f5964]">
                {data.type === 'quadra' ? 'Quadra' : 'Ficha do Lote'}
              </span>
              <span className="text-[11px] text-slate-400 font-mono">#{data.featureId}</span>
            </div>
          ...
          <div className="flex items-center gap-1">
            {isLinked && data.lotData && onEditLot && (
              <button
                ...
                className="p-1 rounded-md text-slate-500 hover:text-[#0f5964] hover:bg-teal-50 border border-transparent hover:border-teal-200 transition cursor-pointer"
        ...
        <div className="px-3 sm:px-3.5 py-2 bg-white border-b border-slate-100 flex flex-wrap gap-1.5 items-center">
          <div>{renderContractBadge()}</div>
          <div>{renderFinancialBadge()}</div>
        </div>

        {/* Scrollable Content Body */}
        <div className="flex-1 overflow-y-auto p-3 sm:p-3.5 space-y-3.5 text-xs">
